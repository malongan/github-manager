...
For advanced web scraping needs, this workspace has **Scrapling** (v0.4.8) installed.

**When to use Scrapling vs browser_use:**

| Scenario | Tool |
|----------|------|
| Simple static pages | `scrapling.Fetcher` (fast, no browser overhead) |
| JS-rendered pages | `scrapling.DynamicFetcher` |
| Anti-bot protected sites (Cloudflare Turnstile etc.) | `scrapling.StealthyFetcher` (headless Chrome with stealth fingerprints) |
| Complex interactive scraping | browser_use |
| Large-scale crawling | Scrapling Spider framework |

**Quick examples:**

```python
# Basic fetcher (fast, impersonates real browser TLS)
from scrapling import Fetcher
p = Fetcher.get('https://example.com')
elements = p.css('h1')

# Stealth fetcher (bypasses Cloudflare Turnstile, bot detection)
from scrapling import StealthyFetcher
p = StealthyFetcher.fetch('https://cloudflare-protected-site.com', 
                           headless=True, network_idle=True)

# Spider for large crawls
from scrapling.spiders import Spider
```

**CLI usage:**
```bash
scrapling extract fetch https://example.com  # basic
scrapling extract stealthy-fetch https://example.com  # stealth
```
